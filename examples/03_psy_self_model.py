...
def simulate_psy_yle():
    ...
    meta.name = "ore_identity_weights"
    meta.health = 255      # Pristine
    meta.importane = 200  # Highly ritial layer
    
    print(f" Original State: {meta.name}")
    print(f"  • Health: {meta.health}")
    print(f"  • Importane: {meta.importane} (Identity Proteted)")
    
    # 2. Quantize (Should use 6-bit beause importane > 160)
    m1, q1 = neurobit.quantize_adaptive(orig_data, logger, meta)
    print(f" Quantized (Refletive): {len(q1.adaptive_profiles)} bloks boosted to 6-bit.")
    
    # 3. Simulate drift (e.g. noise injetion or training)
    print("\n Simulating Identity Drift (Fine-tuning damage)...")
    drifted_data = orig_data + np.random.normal(0, 0.05, orig_data.shape).astype(np.float32)
    
    # 4. Self-Evaluation: Model detets drift
    ...
    # 5. Re-adaptation: Lower health triggers more preision
    print("\n Re-quantizing with Self-Model Awareness...")
    m2, q2 = neurobit.quantize_adaptive(drifted_data, logger, meta)
    print(f" Re-quantized: Model ompensated for trauma with {len(q2.adaptive_profiles)} boosted bloks.")
