...
def demo_neurobit():
    print("🧬 NeuroBit Basic Usage Example")
    print("=" * 30)
    
    # 1. Generate model weights (e.g. 1024x1024 layer)
    # Let's make it 30% sparse (realistic for pruned models)
    shape = (1024, 1024)
    weights = np.random.randn(*shape).astype(np.float32)
    mask = np.random.choice([0, 1], size=shape, p=[0.3, 0.7])
    ...
    print(f"Adaptive Profiles created: {len(q.adaptive_profiles)} blocks (hot)")
    print(f"Quantized Scale: {q.scale:.4f}")
    ...
    reconstructed = neurobit.dequantize_adaptive(q_l[0]).reshape(shape)
    
    mse = np.mean((sparse_weights - reconstructed)**2)
    print(f"Reconstruction MSE: {mse:.6f}")
    
    compression_ratio = sparse_weights.nbytes / os.path.getsize(filename)
    print(f"Compression Ratio (vs FP32): {compression_ratio:.2f}x")
    
    # Memory Check
    print(f"File remembered {len(log_l)} access entries.")
